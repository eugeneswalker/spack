...
def setup_parser(subparser): 
  ...
  subparser.add_argument(
    ...
    required=False,
    default=True,
    help="""embed spec yaml files in generated ci yaml""")
...
def get_jobs(parser, args, **kwargs):
  ...
  y = {
    "variables": {
      "SPACK_ENV_YAML": spack_env_str.replace('$','_DOLLAR_')
    }
  }
  stages = []
  for ii, hs in enumerate(spec_stages):
    ...
    for h in hs:
      ...
      y[job] = {
        ...
        "variables": {
          "SPEC_NAME": job_brief,
          "SPEC_FILE": "./specs/{}.yaml".format(job_brief) if not args.embed_specs else "",
          "SPEC_YAML": "{}".format(s.to_yaml(hash=ht.build_hash)) if args.embed_specs else "",
